...
class resultclass:
	def __init__(self,root):
			...
			title = Label(self.root,text = "add student results",font=("goudy old style", 20),fg="#262626" , bg= "orange").place(x= 10, y=15, width=1180, height=50)
			...
			self.txt_student=ttk.Combobox(self.root,textvariable = self.var_roll,values=self.var_roll_list,font=("goudy old style", 15,"bold"), state= 'readonly', justify = CENTER)
			...
			txt_name = Entry(self.root, textvariable = self.var_name, font= ("goudy old style", 20,"bold"),state ='readonly',bg="lightyellow").place(x=280, y=160, width= 320)
			...
			btn_add=Button(self.root,text= "submit", font = ("times new roman", 15),bg="lightgreen",activebackground ="lightgreen", command = self.add,cursor= "hand2").place(x=300, y=420, width= 120, height= 35)
			btn_clear=Button(self.root,text= "clear", font = ("times new roman", 15),bg="lightgray", activebackground= "lightgray",command= self.clear,cursor= "hand2").place(x=430, y=420, width= 120, height= 35)
	...
	def add(self):
		...
		try:
			if self.var_name.get() == "":
				messagebox.showerror("Error", "record is required", parent= self.root)
			else:
